...
def download_video(url, format_type, quality=None, cookies_path='cookies.txt'):
    ...
    ydl_opts = {
        "format": "bestaudio/best" if format_type == "mp3" else (f"bestvideo[height<={quality}]+bestaudio/best" if quality else "bestvideo+bestaudio"),
        "postprocessors": [{
            "key": "FFmpegExtractAudio",
            "preferredcodec": "mp3",
            "preferredquality": "192",
        }] if format_type == "mp3" else [],
        "outtmpl": os.path.join(download_folder, f"%(title).15s_{uuid.uuid4().hex[:4]}.%(ext)s"),
...
def main():
    ...
    quality = st.selectbox("Escolha a qualidade do vídeo", ("360", "480", "720", "1080")) if format_type == "mp4" else None
    ...
    if st.button("Baixar Todos"):
        if urls_text:
            ...
            if download_paths:
                ...
                logger.info("Todos os downloads bem-sucedidos foram concluídos.")
                if failed_downloads:
                    st.warning(f"Alguns vídeos não foram baixados. Total de falhas: {len(failed_downloads)}")
                ...
                st.error("Não foi possível baixar os vídeos. Verifique as URLs e tente novamente.")
